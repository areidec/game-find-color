...
const Board = () => {
	let [tiles, upadateTiles] = useState(colors);
	let [openedTiles, updateOpenedTiles] = useState([]);
	let [passedTiles, updatePasedTiles] = useState([]);
	...
	const onClickTile = (id) => {
		if(openedTiles.length < 2) {
			...
			let newTile = {};
			let newTiles = tiles.map((tile, idx) => {
				if(tile.id === id) {
					tile.opened = true;
